...
export default function ServiceCard({ title, description, icon: Icon, iconName, isActive }: ServiceCardProps) {
  // Use the icon name to get the component if provided
  const LucideIcon = iconName ? (Icons as any)[iconName] : null;
  
  return (
    <div className={`h-full p-6 bg-white border rounded-xl shadow-sm transition-all ${
      isActive 
